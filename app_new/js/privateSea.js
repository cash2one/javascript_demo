$(function(){
    ...
    var OMS = {
        ...
        renderCusListLi: function(data){
            ...
            for(var i = 0, len = data.length; i < len; i++){
                ...
                htmlTpl += '<h4 class="ui-nowrap itemtoggle" style="overflow: auto;white-space: normal;text-overflow: initial;">' + data[i].cusname + (data[i].is_on_top === 100?"<em class=\"toTop\">置顶</em>":"");
                ...
                htmlTpl += '<span class="visiting">' + data[i].safedays + '天</span>' +
                '</h4>';
                htmlTpl += '<p class="itemtoggle">';

                if (data[i].pullTimeDays > -1)
                    htmlTpl += '<span>跟进' + data[i].pullTimeDays + '天&nbsp;</span>|';
        ...
        renderClassify: function(data){
            ...
            if((!data || !data.length)){
                ...
                if(OMS.filterObj.orderType === 6) {
                    ...
                    classObj = deepCopy(filter.follObj);
                }
        ...
        initTitleBar: function(){
            var title = getUrlParam("title") || "新签客户";
            ...
            else if(this.filterObj.isSub === 1)
                ddbanner.changeBannerTitle(title);
        },
    ...
    var filter = {
        ...
        checkOwnerObj: function(data){
            ...
            localStorage.setItem(filter.key, JSON.stringify(filter.ownerObj));
            filter.renderOwner();
            $('#select-filt').show();
        },
