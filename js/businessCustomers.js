...
var customer = {
    ...
    submitData: function() {
        ...
        var industry = $.trim($("select[name=industry]").val());
        var companyLevel = $.trim($("select[name=companyLevel]").val());
        var sourceId = $.trim($("select[name=sourceId]").val());
        var moneyForecast = $.trim($("input[name=moneyForecast]").val());
        if(OMS_COM.checkEmptyField(cusname, '请填写公司名称')===false) return;
        if(OMS_COM.checkEmptyField(industry, '请选择行业')===false) return;
        if(OMS_COM.checkEmptyField(companyLevel, '请选择客户等级')===false) return;
        ...
        _(customer.idSet).forEach(function(value) {
            ...
            if(OMS_COM.checkEmptyField(temp_name, '请填写联系人')===false) return;
            if(OMS_COM.checkEmptyField(temp_phone, '请填写移动电话')===false) return;
            if(OMS_COM.checkEmptyField(temp_position, '请填写联系人职务')===false) return;
            if(OMS_COM.checkNumField(temp_phone, '电话必须为数字！')===false) return;
    ...
    addContact: function() {
        ...
        $("#qfcontact" + contact_count).after('<div id="qfcontact' + contact_name + '"><div class="ui-form-item-blank" style="background-color: #f5f5f6; width: 100%"></div>' + '<div class="ui-form ui-form-gap form-group" >' + '<div class="ui-form-item ui-form-item-l  ui-border-b"><label>联系人姓名</label><input type="text" name="name_' + contact_name + '" id="name_' + contact_name + '" placeholder="请输入"></div>' + '<div class="ui-form-item ui-form-item-l  ui-border-b"><label>联系人电话</label><input type="tel" name="phone_' + contact_name + '" id="phone_' + contact_name + '" placeholder="请输入"></div>' + '<div class="ui-form-item ui-form-item-l  ui-border-b"><label>联系人职务</label><input type="text" name="position_' + contact_name + '" id="position_' + contact_name + '" placeholder="请输入"></div>' + '<div><ul class="ui-list ui-list-text ui-list-cover ui-row-flex form-group">' + '<li class="ui-col ui-col ui-flex" id="delete_' + contact_name + '"  onclick="customer.delete_contacts(' + contact_name + ')"><i id="_add_relation_" class="ui-icon-list_delete" style="font-size: 15px;color: #ec564d"></i><div class="add-content">删除</div></li>'
    ...
    initApi: function() {
        ...
        var texta = customer.code == 'new' ? '录入企业客户信息' : '编辑企业客户信息';
        dd.ready(function() {
        ...
        OMS_COM.ajaxPost({
            ...
            success: function(data) {
                ...
                if (parseInt(res.code) === 0) {
                    OMS_COM.fillSelect(res.data.industry, "#industry");
                    OMS_COM.fillSelect(res.data.source, "#sourceId");
                    OMS_COM.fillSelect(res.data.companyLevel, "#companyLevel");
                } else {
