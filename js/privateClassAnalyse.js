$(function(){
    ...
    var OMS = {
        ...
        initParams: function(){
            /****初始化列表新增****/
            this.newCustomer = [];
            this.customerKey = "oms_customer_new";
            try {
            ...
            this.filterObj.organization = [];
            this.filterObj.order = 'expire_date';
            /****初始化接口参数 END****/
        ...
        refreshData: function(){
            ...
            var month=localStorage.getItem('team.privateClassAnalyse.dat') || defaultValue;
            OMS.getPrivateClassReport(month,'month', [], []);
        },
        getData: function(){
            $("#msg").html("正在加载中...")
            $("#msg").show();
        ...
        renderCusTable: function(data){
            ...
            for(var i = 0, len = data.length; i < len; i++){
                ...
                htmlTpl += '<span>&nbsp;' + data[i].level + '&nbsp;</span>|';
        ...
        initTable: function(){
                        ...
                        onSuccess : function(result) {
                            dd.biz.util.datepicker({
                                ...
                                onSuccess: function(result) {
                                    ...
                                    localStorage.setItem('team.privateClassAnalyse.dat', monthvalue);
                                    //重置 rightMenu
                                    resetRight(monthvalue);
                                    //加载数据
                                    OMS.getPrivateClassReport(monthvalue,dayType,[],[]);
                                }
                            });
                        }
                    });
            ...
            OMS.getPrivateClassReport(month,dayType, [], []);
        },
        getPrivateClassReport: function (day, dayType, userList, userStatus) {
            var orgIds = OMS.filterObj.organization;
            var orgIdarr = orgIds==''||orgIds==null?[]:JSON.stringify(orgIds);
            OMS_COM.ajaxPost({
                api: 'getPrivateTeachAnalyseData',
                data: {
                    userId: JSON.parse(getCookie('omsUser')).id,
                    // orgid: orgId,
                    order: OMS.filterObj.order,
                    orgList: orgIdarr,
                    userList: userList,
        ...
        setTableData: function (data) {
            ...
            var tableColumn="";
            var lefttable="";
            // lefttable += '<tr><td style="max-width:100px;">会员姓名</td></tr>';
            ...
            for(var i in tableData){
                ...
                if(tableData[i].isClick==1){
                    dept='<a href="privateClassAnalyse.html?orgid='+tableData[i].id+'" >'+dept+'</a>';
                }
                ...
                $.each(Object.keys(OMS.nameMap), function (i, val) {
            ...
            var head = '<thead>\
            <tr>\
            <th style="max-width:100px;" class="table-title">会员姓名</th>';
            $.each(OMS.nameMap, function (i, val) {
                head += '<th>'+val+'</th>';
            });
            head += '</tr>\
            </thead>';
        ...
        scrollTable: function () {
            ...
            $(".table-body").scroll(function (){
                ...
                // if($("#chart-container").css('display')=='none'){
                    $("#lefttable").offset({ top: -1*this.scrollTop + 86 });
                // }else{
    ...
    var filter = {
        ...
        initParams: function(){
            // 筛选默认项
            // 初始化筛选数据对象
            this.key = 'filter_organization';
            var organization = localStorage.getItem(this.key);
            this.orgObj = organization ? (JSON.parse(organization) || []) : [];
            // test
            this.orgObj = {1:'111',2:'222'};

            // 按组织架构分类筛选项目
            this.parttern = "organization";
            this.mapping = {
                "organization": "组织架构"
            };
        ...
        renderOrganization: function () {
            var orgObj = this.orgObj,
                htmlTpl = '',
                self = this;
            htmlTpl = '<ul class="ui-list ui-list-text ultwo selshow" id="level" style="display: ' + (self.parttern == 'organization'?'block':'none') + '">';
            htmlTpl += '<li id="sel_all_level" class="ui-border-t"><p class="filterli">不限</p></li>';
            for(var key in orgObj) {
                htmlTpl += '<li data-name="'+ orgObj[key] +'" data-id="'+ key +'" class="ui-border-t">';
                if(OMS.filterObj.organization && OMS.filterObj.organization.toString().indexOf(key) > -1){
                    htmlTpl += '<p class="filterli activeli">'+orgObj[key]+'</p>';
                }else{
                    htmlTpl += '<p class="filterli">'+orgObj[key]+'</p>';
                }
                htmlTpl += '</li>';
            }
            htmlTpl += '</ul>';
            ...
            $('#level li').on('click',function(event){
                if($(this).attr('id') == 'sel_all_level'){
                    ...
                    if(self.codeInSelected(code, 'organization')){
                        OMS.filterObj.organization.splice(OMS.filterObj.organization.indexOf(code), 1);
                        self.resetFilterSpan({type:'organization',name:name,code:code}, true);
                    }else{
                        OMS.filterObj.organization.push(code);
                        self.resetFilterSpan({type:'organization',name:name,code:code}, false);
                    }
                }
        ...
        renderCity: function(){
            var cityObj = this.cityObj,
                htmlTpl,
                self = this;
            htmlTpl = '<ul class="ui-list ui-list-text ultwo selshow" id="city" style="display: ' + (self.parttern == 'city'?'block':'none') + '">';
            htmlTpl += '<li id="sel_all_city" class="ui-border-t"><p class="filterli">不限</p></li>';
            for(var key in cityObj) {
                htmlTpl += '<li data-name="'+ cityObj[key] +'" data-id="'+ key +'" class="ui-border-t">';
                if(OMS.filterObj.city && OMS.filterObj.city.toString().indexOf(key) > -1){
                    htmlTpl += '<p class="filterli activeli">'+cityObj[key]+'</p>';
                }else{
                    htmlTpl += '<p class="filterli">'+cityObj[key]+'</p>';
                }
                htmlTpl += '</li>';
            }
            htmlTpl += '</ul>';
            ...
            $('#city li').on('click',function(event){
                if($(this).attr('id') == 'sel_all_city'){
                    if(!$(this).find('.filterli').hasClass('activeli')){
                        OMS.filterObj.city = [];
                        $("#city .activeli").removeClass('activeli');
                        $(".level").remove();
                        self.initOmsTop();
                    }
                }
                else {
                    $("#sel_all_city .filterli").removeClass('activeli');
                    var name = $(this).data('name');
                    var code = $(this).data('id');
                    filter.littleCorrect();
                    if(self.codeInSelected(code, 'city')){
                        OMS.filterObj.city.splice(OMS.filterObj.city.indexOf(code), 1);
                        self.resetFilterSpan({type:'city',name:name,code:code}, true);
                    }else{
                        OMS.filterObj.city.push(code);
                        self.resetFilterSpan({type:'city',name:name,code:code}, false);
                    }
                }
                $(this).find('.filterli').toggleClass('activeli');
                stopEventBubble(event);
        ...
        renderThrowReason: function(){
            var throwReasonObj = this.throwReasonObj,
                htmlTpl,
                self = this;
            htmlTpl = '<ul class="ui-list ui-list-text ultwo selshow" id="throwReason" style="display: ' + (self.parttern == 'throwReason'?'block':'none') + '">';
            htmlTpl += '<li id="sel_all_throwReason" class="ui-border-t"><p class="filterli">不限</p></li>';
            for(var key in throwReasonObj) {
                htmlTpl += '<li data-name="'+ throwReasonObj[key] +'" data-id="'+ key +'" class="ui-border-t">';
                if(OMS.filterObj.throwReason && OMS.filterObj.throwReason.toString().indexOf(key) > -1){
                    htmlTpl += '<p class="filterli activeli">'+throwReasonObj[key]+'</p>';
                }else{
                    htmlTpl += '<p class="filterli">'+throwReasonObj[key]+'</p>';
                }
                htmlTpl += '</li>';
            }
            htmlTpl += '</ul>';
            $('#select-filt .ui-col-67').append(htmlTpl);
            //绑定事件
            $('#throwReason li').on('click',function(event){
                if($(this).attr('id') == 'sel_all_throwReason'){
                    if(!$(this).find('.filterli').hasClass('activeli')){
                        OMS.filterObj.throwReason = [];
                        $("#throwReason .activeli").removeClass('activeli');
                        $(".level").remove();
                        self.initOmsTop();
                    }
                }
                else {
                    $("#sel_all_throwReason .filterli").removeClass('activeli');
                    var name = $(this).data('name');
                    var code = $(this).data('id');
                    filter.littleCorrect();
                    if(self.codeInSelected(code, 'throwReason')){
                        OMS.filterObj.throwReason.splice(OMS.filterObj.throwReason.indexOf(code), 1);
                        self.resetFilterSpan({type:'throwReason',name:name,code:code}, true);
                    }else{
                        OMS.filterObj.throwReason.push(code);
                        self.resetFilterSpan({type:'throwReason',name:name,code:code}, false);
                    }
                }
                $(this).find('.filterli').toggleClass('activeli');
                stopEventBubble(event);
        ...
        littleCorrect: function() {
            ...
            if (typeof OMS.filterObj.city == 'string') {
                OMS.filterObj.city = JSON.parse(OMS.filterObj.city);
            }
            if (typeof OMS.filterObj.throwReason == 'string') {
                OMS.filterObj.throwReason = JSON.parse(OMS.filterObj.throwReason);
            }
        }
    };
    var search = {
                ...
                '<div class="ui-searchbar">' +
                    ...
                    '<div class="ui-searchbar-input">' +
                        '<form action="javascript:void(0)" onsubmit="openSearch();">' +
                            '<input value="" type="search" placeholder="搜索客户名称、手机、区域" autocapitalize="off">' +
                        '</form>' +
                    '</div>' +
                    '<i class="ui-icon-close"></i>' +
                '</div>' +
    ...
    var openCustomerInfo = function(code){
        ...
        openLink(oms_config.baseUrl + "customerDetail.html?code=" + code + "&type="+OMS.filterObj.type);
    };
    ...
    home.prototype.getCustomers = function(obj){
        ...
        if(typeof form_data['user'] == "object")
            form_data['user'] = JSON.stringify(form_data.user);
        if(typeof form_data['level'] == "object")
            form_data['level'] = JSON.stringify(form_data.level);
        if(typeof form_data['city'] == "object")
            form_data['city'] = JSON.stringify(form_data.city);
        if(typeof form_data['throwReason'] == "object")
            form_data['throwReason'] = JSON.stringify(form_data.throwReason);
        OMS_COM.ajaxPost({
            api: 'getCustomerList',
            data: form_data,
            success: obj.success,
