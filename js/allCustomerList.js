    ...
    var OMS = {
        ...
        initParams: function(){
            ...
            if(this.filterObj.type == 2){
                this.filterObj.order = 'privateTeach';
                this.sortList = {
                    'privateTeach': '已购私教课',
                    'follow': '已跟进',
                    'notFollow': '未跟进'
        ...
        renderCusListLi: function(data){
            ...
            for(var i = 0, len = data.length; i < len; i++){
                ...
                //     }
                // }
                htmlTpl += '<span class="visiting">' + (parseInt(data[i].lastFollow)==0?'未跟进':(data[i].lastFollow+'天')) + '</span>' +
                '</h4>';
            ...
            OMS_COM.ajaxPost({
               api: 'getCustomerConfig',
                ...
                success: function (data) {
                    var res = JSON.parse(data);
                    if(parseInt(res.code)==0) {
                        _.forEach(Object.keys(res.data.level), function (val) {
                            _self.levelObj[val] = res.data.level[val];
                        });
                        if(OMS.filterObj.type == 4){
                            _.forEach(Object.keys(res.data.city), function (val) {
                                _self.cityObj[val] = res.data.city[val];
                            });
                            _.forEach(Object.keys(res.data.throwReason), function (val) {
                                if(res.data.throwReason[val]!='不限')
                                    _self.throwReasonObj[val] = res.data.throwReason[val];
                            });
                        }
                    }
                    filter.renderLevel();
