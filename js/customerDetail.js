...
var cusInfo = {
    ...
    renderHeader: function (data) {
        $("#tab-header-content").append('<li class="first-banner"><div class="ui-flex ui-flex-pack-center ui-nowrap"><h4>' + cusInfo.switchNull(data) + '</h4></div></li>');
    },
    renderFooter: function () {
        var contents = '';
        var role = JSON.parse(getCookie("omsUser")).role;
        if(cusInfo.type == 4) {
            contents = '<ul class="ui-tiled ui-border-t" style="background-color:#f5f5f6;">' +
                '<li class="startVisit" id="startVisit" onclick="startToCustomerPull()">' +
                '<div class="ui-list-thumb icon-set-block">' +
                '<i class="ui-icon-start_interview" style="font-size: 27px;"></i>' +
                '</div>' +
                '<div class="footer-font">&nbsp;拉入客户</div></li>';
            contents += '</ul>';
        } else {
            contents = '<ul class="ui-tiled ui-border-t" style="background-color:#f5f5f6;">' +
                '<li class="startVisit" id="startVisit" onclick="startToCustomerFollow()">' +
                '<div class="ui-list-thumb icon-set-block">' +
                '<i class="ui-icon-start_interview" style="font-size: 27px;"></i>' +
                '</div>' +
                '<div class="footer-font">&nbsp;保有跟综</div></li>';

            if (JSON.parse(getCookie("omsUser")).role != 4) {//私教角色不能投入公有
                contents += '<li class="cancelLidan" id="cancelLidan" onClick="startToHealthSurvey()">' +
                    '<div class="ui-list-thumb icon-set-block">' +
                    '<i class="ui-icon-toolbar_cancel"></i>' +
                    '</div>' +
                    '<div class="footer-font">&nbsp;健康问卷</div></li>';
            }


            contents += '<li class="" id="bodyAnalyses" onClick="startToBodyAnalyses()">' +
                '<div class="ui-list-thumb icon-set-block">' +
                '<i class="ui-icon-toolbar_arrangelist"></i></div>' +
                '<div class="footer-font">&nbsp;POS</div></li>';
            if (role != 4) {//私教角色不能投入公有
                contents += '<li class="" id="throwConfirm" onClick="startThrow()">' +
                    '<div class="ui-list-thumb icon-set-block">' +
                    '<i class="ui-icon-toolbar_forecast"></i></div>' +
                    '<div class="footer-font">&nbsp;投入公有</div></li>';
            }
            if (role == 4 || role == 5) {//私教角色才能私教跟踪
                contents += '<li class="" id="liDanReturn" onClick="startCoachRecord()">' +
                    '<div class="ui-list-thumb icon-set-block">' +
                    '<i class="ui-icon-tab_payment"></i></div>' +
                    '<div class="footer-font">&nbsp;私教跟踪</div></li>';
            }
            contents += '</ul>';
        }
        $("#main-foot").append(contents);
    },
    ...
    renderInfo: function(data) {
        var hasExperience = '';
        if(cusInfo.switchNull(data.hasExperience)==0)
            hasExperience = '无';
        else
            hasExperience = '有';
        $("#cusBasicInfo").prepend(
            '<ul class="ui-list ui-list-pure ui-border-b" id="cusinfo">' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>客户名称</label>' +
            '<div class="cusInfoContent" style="padding-top:14.5px;padding-bottom:16.5px;line-height:20px">' +
            cusInfo.switchNull(data.name) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi private-content lidan-content role5-content">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>联系方式</label>' +
            '<div class="cusInfoContent" style="float:left;width:100px;padding-right:110px;">' +
            cusInfo.switchNull(data.mobile) +
            '</div>' +
            '<div onclick="businessCall.phoneCall(\'' + data.mobile + '\',\'' + data.mobile + '\',\'' + data.name + '\',\'' + data.name + '\',\'' + cusInfo.code + '\', 0, event)">' + '<i class="ui-icon-list_phone" style="line-height:60px; font-size:20px;color:#ec564d;"></i></div>' +
            '</li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>微信号</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(data.weixinId) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi" id="cusPersonal">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>性别</label>' +
            '<div class="cusInfoContent" id="cusPersonal-content">' +
            cusInfo.switchNull(cusInfo.gender[data.sex]) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>城市</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(data.cityName) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>门店</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(data.shopName) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>活动区域</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(data.activeArea) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>客户生日</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(data.birthday) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>健身需求</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(cusInfo.requirment[data.requirmentId]) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi cusSourceLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>客户来源</label>' +
            '<div class="cusInfoContent">' +
            cusInfo.switchNull(cusInfo.source[data.sourceId]) +
            '</div></li>'+
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>健身史</label>' +
            ...
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>个人评级</label>' +
            '<div class="cusInfoContent " id="myLevelStr">' +
            cusInfo.switchNull(cusInfo.level[data.level]) +
            '</div>' +
            (parseInt(data.type)==0?"":('<div class="phone-div" id="cusMyLevelBtn" onclick="cusInfo.choosePresonal();">' +
            '<i class="ui-icon-list_edit" style="font-size: 18px; color:#ec564d;">' +
            '</i>' +
            '</div>'))+'</li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>预计金额</label>' +
            '<div class="cusInfoContent ">' +
            cusInfo.switchNull(data.moneyForecast) +
            '</div></li>' +
            '<li class="ui-border-t cusInfoLi">' +
            '<div class="ui-form-item ui-form-item-show">' +
            '<label>创建时间</label>' +
            '<div class="cusInfoContent ">' +
            cusInfo.switchNull(data.cTime) +
            '</div></li>' +
            // '<li class="ui-border-t cusInfoLi cusNewSign private-content lidan-content-new" style="display: none">' +
            // '<div class="ui-form-item ui-form-item-show">' +
            // '<label>预计金额</label>' +
            // '<div class="cusInfoContent">' +
            // cusInfo.switchNull(data.moneyForecast) + '</div></li>' +
            // '<li class="ui-border-t cusInfoLi cusNewSign public-content lidan-content-new" style="display: none; height:auto !important">' +
            // '<h4 style="word-break:break-all; padding-top:10px;padding-bottom:10px;font-size:15px;line-height:27px;">' +
            // cusInfo.switchNull(data.remark) + '</h4></li>' +
            // '<li class="ui-border-t cusInfoLi cusRenewal private-content lidan-content-renew" style="display: none">' +
            // '<div class="ui-form-item ui-form-item-show">' +
            // '<label>产品类型</label>' +
            // '<div class="cusInfoContent">' +
            // cusInfo.switchNull(data.new_product_version) +
            // '</div></li>' +
            // '<li class="ui-border-t cusInfoLi cusRenewal private-content lidan-content-renew" style="display: none">' +
            // '<div class="ui-form-item ui-form-item-show">' +
            // '<label>合同到期</label>' +
            // '<div class="cusInfoContent">' +
            // cusInfo.switchNull(data.enddate) +
            // '</div></li>' +
            '</ul>');
    },
    renderContactList: function(data) {
        ...
        if (data.length > 0) {
            var list = "";
            for (var i in data) {
                if(data[i].name == ''&&data[i].phone==''&&data[i].relation=='') continue;
    ...
    renderContactLi: function(obj) {
        var list = '<li class="ui-border-b" style="height:60px;margin-right:15px">' + '<div class="ui-list-info" style="padding-top:15px;">' + '<h4 class="ui-nowrap" style="color:#333; font-size:15px;line-height:1">' + obj.name + ' | ' + cusInfo.relation[obj.relation] + '</h4>' + '<span style="color:#666;font-size:12px;line-height:1">' + obj.phone + '</span></div>';
        if (dd.ios) {
            list += '<div onclick="cusInfo.smsCall(\'' + obj.phone + '\',\'' + obj.phone + '\',event)">' + '<i class="ui-icon-message" style="line-height:60px; font-size:20px;color:#ec564d; padding-right: 15px;"></i></div>'
        }

        list += '<div onclick="businessCall.phoneCall(\'' + obj.phone + '\',\'' + obj.phone + '\',\'' + obj.name + '\',\'' + cusInfo.cusBasicInfo.name + '\',\'' + cusInfo.code + '\', 0, event)">' + '<i class="ui-icon-list_phone" style="line-height:60px; font-size:20px;color:#ec564d;"></i></div>' + '</li>';
        return list;
    },
        ...
        OMS_COM.ajaxPost({
            ...
            success: function(data) {
                ...
                if (parseInt(res.code) === 0) {
                    cusInfo.cusBasicInfo = res.data;
                    cusInfo.status = res.data.status;
                    ...
                    cusInfo.renderHeader(res.data.name);
                    cusInfo.renderFooter();
                    cusInfo.renderContactList(res.data.contacter);
                    cusInfo.renderInfo(res.data);
                } else {
    ...
    initRight: function() {
        ...
        var showRightBtn = false;
        if (JSON.parse(getCookie("omsUser")).role != 4 && cusInfo.type != 2) {
            if(cusInfo.type != 4)
                showRightBtn = true;
        }
        ...
        dd.biz.navigation.setRight({
            ...
            text: '编辑',
            onSuccess: function(result) {
                openLink("personalCustomerAdd.html?code=edit&cusid="+cusInfo.code);
            },
            onFail: function(err) {}
        });
    },
    ...
    initParam: function() {
        OMS_COM.ajaxPost({
            ...
            success: function(data) {
                var res = JSON.parse(data);
                if (parseInt(res.code) === 0) {
                    cusInfo.gender = res.data.gender;
                    cusInfo.requirment = res.data.requirment;
                    cusInfo.source = res.data.source;
                    cusInfo.level = res.data.level;
                    cusInfo.relation = res.data.relation;
                    cusInfo.city = res.data.city;
                    cusInfo.cityName = res.data.cityName;
                    cusInfo.shopName = res.data.shopName;
                } else {
                    // DDCtrl.showAlert("接口提交失败，请联系技术支持！");
                }
            },
        ...
        OMS_COM.ajaxPost({
            ...
            success: function(data) {
                var res = JSON.parse(data);
                if (parseInt(res.code) === 0) {
                    cusInfo.followType = res.data.followType;
                    localStorage.setItem('cusInfo.followType', JSON.stringify(cusInfo.followType));
                    _.forEach(Object.keys(cusInfo.followType), function (val) {
                        $("#typeSelect").prepend("<option value='"+val+"'>"+cusInfo.followType[val]+"</option>");
                    });
                } else {
                    // DDCtrl.showAlert("接口提交失败，请联系技术支持！");
                }
    ...
    renderChooseMylevel: function() {
        var items = '';
        _.forEach(Object.keys(cusInfo.level), function (val) {
            items +=
                '<li class="ui-border-t mylevel-li" data-id="'+val+'">' +
                '<h4 class="ui-nowrap">'+cusInfo.level[val]+'</h4>' +
                '<div class="ui-poi-dist"></div></li>';
        });
        var htmlTpl =
            "<div class='map-poi-container-wrap' id='chooseMylevel'>" +
            "<div class='map-poi-container'>" +
            "<ul class='ui-list ui-list-text ui-list-active ui-border-b form-group' id='chooseMylevel_li'>" +
            // '<li class="ui-border-t mylevel-li" data-id="a">' +
            // '<h4 class="ui-nowrap">a级</h4>' +
            // '<div class="ui-poi-dist"></div></li>' +
            // '<li class="ui-border-t mylevel-li" data-id="b">' +
            // '<h4 class="ui-nowrap">b级</h4>' +
            // '<div class="ui-poi-dist"></div></li>' +
            // '<li class="ui-border-t mylevel-li" data-id="c">' +
            // '<h4 class="ui-nowrap">c级</h4>' +
            // '<div class="ui-poi-dist"></div></li>' +
            // '<li class="ui-border-t mylevel-li" data-id="d">' +
            // '<h4 class="ui-nowrap">d级</h4>' +
                items +
            '<div class="ui-poi-dist"></div></li>' +
            '</ul></div></div>';
        $("body").append(htmlTpl);
        $('[data-id = "' + cusInfo.cusBasicInfo.level + '"]').find('.ui-poi-dist').html('<i class="ui-icon-list_cerrect" style="font-size:20px;color:#ec564d;padding-right:15px"></i>');
                ...
                if(parseInt(res.code) === 0) {
                    dd.device.notification.toast({
                        ...
                        onSuccess: function (result) {
                            ...
                            // $("#myLevelStr").html(cusInfo.level[cusInfo.mylevel]);
                            // $("#chooseMylevel").remove();
                            // cusInfo.initApi();
                            // $("#main-body").show();
                            replaceLink('customerDetail.html?code='+cusInfo.code);
                        },
                        onFail: function (err) {
                        }
                    });
...
function startToCustomerFollow() {
    if(cusInfo.status == 2) {
        var message = '';
        OMS_COM.ajaxPost({
            api: 'checkPrivateFollowRight',
            data: {
            ...
            success: function (data) {
                var res = JSON.parse(data);
                if (parseInt(res.code) === 0) {
                    if (res.data == 1) {//1代表有权限，0代表没权限
                        openLink("customerFollowAdd.html?code=" + cusInfo.code + "&status=" + cusInfo.status);
                    } else {
                        message = '您暂时没有该操作的权限';
                    }
                } else {
...
function startToHealthSurvey() {
    openLink("healthSurvey.html?code="+cusInfo.code+"&status="+cusInfo.status);
}

function startToBodyAnalyses() {
    openLink("bodyAnalysesAdd.html?code="+cusInfo.code+"&status="+cusInfo.status);
}
...
function startCoachRecord() {
    openLink("coachRecordList.html?code="+cusInfo.code+"&status="+cusInfo.status);
}
