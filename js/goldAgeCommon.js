...
var CONSTANT_BILL = {
    ...
    OMS_COM = {
        ...
        fillSelect: function(data, target) {
            _.forEach(Object.keys(data), function(val){
                $(target).append("<option value='"+val+"'>"+data[val]+"</option>");
            });
        },
        checkEmptyField: function(data, message) {
            if (data == "") {
                dd.device.notification.toast({
                    icon: 'error',
                    text: message,
        ...
        checkFloatField: function(data, message) {
            var float_regex = new RegExp(/^\d+(?:\.\d+|)$/);
            if (!float_regex.test(data)) {
                dd.device.notification.toast({
                    icon: 'error',
                    text: message,
                    duration: 1,
        ...
        checkNumField: function(data, message) {
        ...
        ajaxPost: function(config) {
            var apiUrl = oms_config.apiUrl2;
            var sendData = config.data || {};
            const timestamp = Math.floor(Date.now() / 1000);
            sendData['action'] = oms_apiList[config.api];
            sendData['version'] = 1;
            sendData['timestamp'] = timestamp;
            sendData['sign'] = md5(oms_apiList[config.api] + sendData['version'] + timestamp + oms_config.privateKey);
            sendData['omsuid'] = JSON.parse(getCookie("omsUser")).id;
            sendData['token'] = JSON.parse(getCookie("omsUser")).token;
            sendData['session_id'] = JSON.parse(getCookie("omsUser")).session_id;
            $.ajax({
                url: apiUrl,
        ...
        convertFormArrayToObject: function (data) {
            var form_data = {};
