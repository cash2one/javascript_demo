...
(function(exports) {
  ...
  exports.globalSearchPage = new Vue({
        ...
        if (keyword && keyword.length) {
          ...
          return globalSearchStore.search({keyword: keyword}, function(resp) {
            ...
            if (parseInt(res.code) === 0) {
              ...
              // END
              var customers = [];
              $.each(data.customer.list, function(i, item) {
                customers.push(item);
              });
              ...
              var contactors = [];
              $.each(data.contract.list, function(i, item) {
    ...
    filters: {
      ...
      statusStringify: function(status) {
        status = String(status);
        var typeObj;
        globalSearchStore.CUSTOMER_STATUS_CONFIG.forEach(function(obj) {
          if (obj.value == status) {
            return typeObj = obj;
          }
        });
