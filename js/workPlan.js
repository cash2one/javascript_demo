	...
	renderList : function(list,datel){
		...
		if(list== 0 || list==null || list.length === 0){
			...
			for (var i in list) {
				...
				sHtml +='<li class="ui-border-b" style="height:60px;margin-right:15px">' +
					'<div class="ui-list-info" style="padding-top:15px;">' +
					'<h4 class="ui-nowrap" style="color:#333; font-size:15px;line-height:1">' + list[i].startTime + '~' + list[i].endTime.substr(11,5)  + ' | ' + list[i].title + ' | ' + todo.work_type[list[i].type] + '</h4>' +
					'<span style="color:#666;font-size:12px;line-height:1">' + list[i].remark + '</span></div></li>';
				// sHtml += '<li class="ui-border-b" >'
				// 	+ '<div style="height:47px;">'
				// 	+ '<span class="miss_list">'
				// 	+ '<span class="mis_font">' + list[i].startTime + '</span>'
				// 	+ '<span class="mis_color" style="margin-left:12px;">' + list[i].title + '</span>'
				// 	+ '<span class="mis_color" style="margin-left:12px;">' + todo.work_type[list[i].type] + '</span>';
					// if(list[i].type == "拜访") {
