...
var userSelWidget = {
    ...
    keyword: function(input) {
        if (typeof input == 'string') {
            this.$el.find('.userSelWidget_form input').val(input);
        }
        return $.trim(this.$el.find('.userSelWidget_form input').val());
    },
    ...
    search: function() {
        ...
        OMS_COM.ajaxPost({
            ...
            always: function () {
                self.$el.find('.userSelWidget_loading').hide();
            }
        });
    ...
    resetResult: function() {
        this.list = null;
        this.$el.find('.userSelWidget_result').empty();
        this.$el.find('.userSelWidget_loading').hide();
        this.$el.find('.userSelWidget_noresult').hide();
    },
    ...
    renderResult: function(data) {
        ...
        if (this.list && this.list.length) {
            this.$el.find('.userSelWidget_result').append(html);
        } else {
            if (html) {
                this.$el.find('.userSelWidget_result').html(html);
            } else {
                this.$el.find('.userSelWidget_noresult').show();
            }
        }
    },
    ...
    reset: function(mode, filter, callback) {
        ...
        $.extend(this, {
            mode: mode,
            filter: $.extend({}, userSelWidget._defaultFilter, filter),
            callback: callback
        });
        this.list = null;
        this.item = null;
    ...
    loading: function() {
        var self = this;
        this.resetResult();
        this.$el.find('.userSelWidget_loading').show();
    ...
    loadingMore: function() {
        ...
        $loadingm.appendTo(this.$el.find('.userSelWidget_result'));
        this.search().always(function() {
            $loadingm.remove();
        });
    },
    ...
    togglePanel: function(tosearch) {
        if (tosearch) {
            this.$el.find('.userSelWidget_search').hide();
            this.$el.find('.userSelWidget_result').show();
        } else {
            var input = this.keyword();
            this.$el.find('.userSelWidget_result').hide();
            if (input.length) {
                this.$el.find('.userSelWidget_search').show();
                this.$el.find('.userSelWidget_searchTxt').text(input);
            } else {
                this.$el.find('.userSelWidget_search').hide();
            }
            console.log('to here');
    ...
    selectItem: function(event) {
        var $li = $(this), self = userSelWidget;
        var $searchresult = self.$el.find('.userSelWidget_result'),
    ...
    bindEvents: function() {
        var self = this;
        // input
        var $searchform = this.$el.find('.userSelWidget_form');
        function redosearch() {
            self.togglePanel(true);
            self.loading();
            if (!self.keyword().length) {
                $searchform.find('input').blur();
                $searchform.find('.userSelWidget_input').removeClass('focus');
            }
        }
        ...
        $searchform.find('.userSelWidget_input').on('tap', function() {
            $(this).addClass('focus');
        ...
        // select
        this.$el.find('.userSelWidget_result').on('click', 'li', this.selectItem);
        // scroll loadingMore
    ...
    render: function(){
        var html = '';
        html += '<div id="userSelWidget_wrapper">';
        html += '    <form class="userSelWidget_form">';
        html += '        <div class="userSelWidget_input ui-searchbar-wrap ui-border-b">';
        ...
        html += '        </div>';
        html += '    </form>';
        html += '    <div class="userSelWidget_search" style="display:none">';
        ...
        html += '                 <h4 class="ui-nowrap">搜索“<span class="userSelWidget_searchTxt" style="color:#e15151;"></span>”</h4>';
        html += '              </div>';
        html += '          </li>';
        html += '       </ul>';
        html += '    </div>';
        html += '    <div class="userSelWidget_loading ui-center" style="display:none;">';
        html += '        <div class="ui-loading-wrap">';
        html += '            <i class="ui-loading"></i>';
        html += '        </div>';
        html += '    </div>';
        html += '    <div class="userSelWidget_noresult ui-center" style="display:none;">';
        html += '        <p>没有找到相关客户</p>';
        html += '    </div>';
        html += '    <ul class="ui-list ui-list-text ui-list-one userSelWidget_result"></ul>';
        html += '</div>';
    ...
    initNav: function() {
        var self = this;
        dd.ready(function() {
            dd.biz.navigation.setTitle({title: '选择跟进人'});
            if (self.mode == 'normal') {
